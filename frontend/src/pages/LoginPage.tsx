    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--surface-raised)] px-4">
      ...
      <div className="relative w-full max-w-sm animate-fade-up">
        <div className="card p-8 text-center">
          ...
          {errorMsg && (
            <div className="mb-5 px-4 py-3 rounded-[var(--radius-sm)]
              bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800
              text-red-700 dark:text-red-400 text-sm text-left">
              ⚠️ {errorMsg}
            </div>
          )}

          <button onClick={login} disabled={isLoading} className="btn-primary w-full py-3 text-base">
            {isLoading ? (
              ...
              <><span>Anmelden mit SSO</span><span className="text-white/70">→</span></>
            )}
          </button>
